...
import { PRICE_POLL_MS, SPARK_WINDOW_MS, clamp, formatCoverage, formatUsd } from './utils';
...
// IMPORTANT: your file might export default OR named.
// Your earlier build error said "has no default export".
// Your current code uses named import. Keep it named:
 ...
 * TS FIX: cleanup must return void, never `0` (so no `raf && cancelAnimationFrame(raf)`).
 */
...
export default function JackpotPanel({
  ...
  // NOTE: your hook uses momentumGlobalH1 (per your earlier TS error)
  ...
  // Your hook return shape has been changing while refactoring.
  // Keep it build-safe without breaking runtime:
  const priceSamples = usePriceSamples(priceUsd) as any;

  const range24h = (priceSamples?.range24h ?? null) as
    | { lowUsd: number; highUsd: number }
    | null;
  const coverageMs = (priceSamples?.coverageMs ?? 0) as number;
  const spark = (priceSamples?.spark ?? null) as { points: string; min: number; max: number } | null;
  const sparkCoverageMs = (priceSamples?.sparkCoverageMs ?? 0) as number;

  // This is the one that keeps disappearing in your hook and breaking builds.
  const maxJackpotToday = (priceSamples?.maxJackpotToday ?? null) as number | null;

  const registerJackpotUsdForSessionPeak = (priceSamples?.registerJackpotUsdForSessionPeak ??
    (() => {})) as (usd: number) => void;

  // --- Panel layout auto-wide
          ...
          <div className="relative mt-5 grid gap-4">
            ...
            >
              ...
              <div className="mt-3 flex flex-wrap items-center justify-center gap-3 sm:justify-start">
                ...
                >
                  {mounted ? new Date(Math.max(0, countdownMs)).toISOString().substr(11, 8) : '00:00:00'}
                </span>
