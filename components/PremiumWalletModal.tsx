...
import { ChevronRight, ExternalLink, ShieldCheck, Wallet, AlertTriangle, Loader2 } from 'lucide-react';
...
function prettyErr(e: any) {
  ...
  if (!msg) return 'Could not connect wallet.';
  // Keep it short + premium
  if (msg.toLowerCase().includes('user rejected')) return 'Connection request was rejected.';
  if (msg.toLowerCase().includes('wallet not ready')) return 'Wallet not ready. Please open it and try again.';
...
export default function PremiumWalletModal({
  ...
  const { wallets, wallet, publicKey, connected, connecting, disconnect, select, connect } = useWallet();
  ...
  // Close once we actually become connected
  useEffect(() => {
  ...
  async function handlePick(name: WalletName) {
    ...
    try {
      ...
      // Let adapter hydrate selection
      await new Promise((r) => setTimeout(r, 60));

      // Trigger connect flow (shows wallet approval UI)
      await connect();
    } catch (e: any) {
      setError(prettyErr(e));
    } finally {
      window.setTimeout(() => setBusy(null), 220);
    }
  }
        ...
        >
          ...
          >
            ...
            <div className="relative p-5 sm:p-6">
              ...
              {/* Title */}
              <div className="mt-4">
                <h3 className="text-xl font-semibold text-slate-100">Select a wallet to enter XPOT</h3>
                <p className="mt-1 text-sm text-slate-300">Fast, simple and clean. You can change it anytime.</p>
              </div>
              ...
              <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 font-mono text-xs text-slate-400">{address ? shortAddr(address) : 'Not connected'}</p>
                  </div>
                  ...
                  {connected ? (
                    ...
                    <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-2">
                      ...
                      <span className="text-[11px] font-semibold text-slate-200">Secure</span>
                    </div>
                  )}
                </div>

                {connecting ? <p className="mt-2 text-xs text-amber-200">Waiting for wallet approval…</p> : null}
              </div>

              {/* Error (only AFTER attempted) */}
              {attempted && error ? (
                <div className="mt-3 rounded-2xl border border-rose-500/20 bg-rose-500/10 px-4 py-3">
                  <div className="flex items-start gap-2">
                    <AlertTriangle className="mt-0.5 h-4 w-4 text-rose-200" />
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-rose-100">Could not connect</p>
                        ...
                        <div className="flex items-center gap-2">
                          {!installed && url ? (
                            <a
                              ...
                              className="hidden sm:inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-[11px] font-semibold text-amber-200 hover:bg-white/[0.07]"
                            >
              ...
              <div className="mt-4 text-[11px] text-slate-500">We never see your seed phrase.</div>
            </div>
          </motion.div>
        </motion.div>
      ) : null}
