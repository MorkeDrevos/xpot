...
// For now: treat *one* of the entries as winner
const mockEntries: Entry[] = Array.from({ length: entryCount }).map(
  (_, i) => ({
    id: i + 1,
    code: makeCode(i),
    status: 'in-draw',
...
// Mark first entry as current winner (mock)
if (mockEntries.length > 0) {
  mockEntries[0].status = 'won';
}
...
<section className="mt-6 rounded-2xl border border-slate-700 bg-slate-900/60 p-4">
  ...
  <p className="mt-1 text-xs text-slate-400">
    In v1, you'll connect a Solana wallet so your XPOT balance can update entries in real time.
  </p>
  <button
    type="button"
...
<section className="mt-10 rounded-2xl border border-emerald-500/40 bg-slate-900/70 p-6 shadow-[0_0_60px_rgba(16,185,129,0.18)]">
  <h2 className="text-lg font-semibold text-emerald-100">Today’s result</h2>

  {entries.some(e => e.status === 'won') ? (
    <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <p className="text-sm text-slate-300">
          One of your codes <span className="font-mono text-emerald-300">
          {entries.find(e => e.status === 'won')?.code}
          </span>{' '}
          hit today’s jackpot.
        </p>
        <p className="mt-1 text-xs text-slate-400">
          Return to your dashboard after the draw to claim. If you don’t claim in time,
          the unclaimed amount rolls over on top of the next jackpot.
        </p>
      </div>
      ...
      <button
        ...
        onClick={() => {
          ...
          setEntries(prev =>
            prev.map(e =>
              e.status === 'won' ? { ...e, status: 'claimed' } : e
            )
          );
        }}
    ...
    <p className="mt-4 text-sm text-slate-300">
      Your codes are in the draw. The result will appear here when the timer hits zero.
    </p>
  )}

  {winnerClaimed && (
    <p className="mt-3 text-xs text-emerald-300">
      Nice catch. Any unclaimed portion would roll over on top of tomorrow’s jackpot.
    </p>
  )}
</section>
  ...
  <p className="mt-1 text-xs text-slate-500">
    Each code represents one chance to win in a specific draw. Entries are generated based on your XPOT balance.
  </p>
