...
// ─────────────────────────────────────────────
// ✅ Balance formatting (FIXES “broken balances”)
// - Handles massive uiAmountString or raw amount safely
// - Adds commas without using Number() for huge values
// ─────────────────────────────────────────────
function addCommasToNumericString(input: string) {
  const s = (input ?? '').trim();
  if (!s) return null;
  ...
  // Must look like digits[.digits]
  if (!/^\d+(\.\d+)?$/.test(body)) return input;

  const [whole, frac] = body.split('.');
  const wholeWith = whole.replace(/\B(?=(\d{3})+(?!\d))/g, ',');
  const out = frac != null && frac.length ? `${wholeWith}.${frac}` : wholeWith;
...
function formatVaultBalance(b: ApiVaultEntry['balance']) {
  ...
  // Prefer uiAmount when it exists and is finite (safe for normal sizes)
  if (typeof b.uiAmount === 'number' && Number.isFinite(b.uiAmount)) {
    return formatMaybeNumber(b.uiAmount) ?? null;
  }

  // uiAmountString is often present for large values (safe string) - add commas safely
  if (typeof b.uiAmountString === 'string' && b.uiAmountString.trim().length) {
    return addCommasToNumericString(b.uiAmountString) ?? b.uiAmountString;
  }

  // Fallback: raw amount + decimals (BigInt math) then add commas
  if (typeof b.amount === 'string' && typeof b.decimals === 'number') {
    const s = formatRawAmount(b.amount, b.decimals);
    if (!s) return null;
    return addCommasToNumericString(s) ?? s;
  }
...
// ... (rest of your file unchanged from your paste)
// NOTE: Everything below this point is exactly your content; no logic changes needed there.
...
// Team vesting panel remains exactly as your paste:
...
function TokenomicsPageClient() {
  ...
  // ---- Your JSX below is unchanged from your paste ----
  return (
    <XpotPageShell
      title="Tokenomics"
