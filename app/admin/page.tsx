...
const AUTO_DRAW_ENABLED = process.env.NEXT_PUBLIC_XPOT_AUTO_DRAW_ENABLED === 'true';
...
function UsdPill({ amount, size = 'md' }: { amount: number | null | undefined; size?: 'sm' | 'md' }) {
...
function XpotPill({ amount, size = 'md' }: { amount: number | null | undefined; size?: 'sm' | 'md' }) {
...
function XpotLogoAnimated({ className, width = 132, height = 36 }: { className?: string; width?: number; height?: number }) {
...
export default function AdminPage() {
  ...
  async function authedFetch(input: string, init?: RequestInit) {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Request failed (${res.status}): ${body || res.statusText || 'Unknown error'}`);
    }
  ...
  async function handlePickMainWinner() {
    ...
    try {
      ...
      setPickSuccess(`Main XPOT winner: ${winner.ticketCode} (${winner.walletAddress.slice(0, 4)}…${winner.walletAddress.slice(-4)}).`);
  ...
  return (
    <XpotPageShell title="Operations Center" subtitle="Control room for today’s XPOT">
      {/* Background layers (ok to keep here if you want admin to be extra cinematic) */}
      <div className="pointer-events-none fixed inset-0 -z-10 bg-[#02020a]" />
      <div
        className="
          pointer-events-none fixed inset-0 -z-10 opacity-95
      ...
      <main className="relative z-10 mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col items-start gap-1 sm:items-end">
            <div className="flex flex-wrap items-center gap-2 sm:justify-end">
              <h1 className="text-sm font-semibold text-white sm:text-base">Control room for today’s XPOT</h1>
            ...
            <div className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-5">
              ...
              <form onSubmit={handleUnlock} className="flex flex-1 flex-col gap-2 sm:max-w-xl sm:flex-row">
                ...
                <div className="flex gap-2">
                  <button type="submit" disabled={isSavingToken || !tokenInput.trim()} className={`${BTN_UTILITY} px-4 py-2 text-xs`}>
                    {tokenAccepted ? 'Update key' : 'Unlock'}
                  </button>
                  {tokenAccepted && (
                    <button type="button" onClick={handleClearToken} className={`${BTN_UTILITY} px-4 py-2 text-xs`}>
                      Clear
                    </button>
                  )}
                </div>
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,3fr)_minmax(0,2fr)]">
          ...
          <div className="space-y-4">
            ...
            {/* Schedule bonus XPOT */}
            {/* (left as-is – your UI is good; only structure fixes were needed above) */}
            {/* ...keep the rest of your bonus + tickets sections exactly like you pasted... */}
            {/* For brevity in this message: you can keep them unchanged from your paste. */}
            {/* If you want, I’ll re-paste the remainder too, but it’s already clean. */}
          </div>

          {/* RIGHT column winners */}
          {/* (same: keep your winners block as you pasted) */}
        </section>
      </main>
